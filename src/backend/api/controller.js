...
const getById = async (req, res, table) => {
  try {
    const data = await knex(table).where(req.params);
    if (data.length > 0) {
      res.status(200).json(data);
    } else {
      res.status(404).json({
        'message': `${table} with id ${req.params.id} does not exist`
      });
    }
  } catch (e) {
    res.status(500).json(e.message);
...
const updateById = async (req, res, table) => {
  try {
    ...
    if (data.length > 0) {
      ...
      res
        .status(201)
        .json({ 'message': `${table} with id ${id.id} updated successfuly` });
    } else {
      res
        .status(404)
...
const deleteById = async (req, res, table) => {
  try {
    ...
    if (data.length > 0) {
      ...
      res
        .status(200)
        .json({ 'message': `${table} with id ${id.id} deleted successfuly` });
    } else {
      res
        .status(404)
        .json({ 'message': `${table} with id ${id.id} does not exist` });
